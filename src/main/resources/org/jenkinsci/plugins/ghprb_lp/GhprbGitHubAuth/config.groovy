...
f.entry(title: _("Proxy"), field: "serverAPIProxy") {
    f.textbox()
}
...
f.advanced(title: _("Test Credentials")) {
    f.optionalBlock(title: _("Test basic connection to GitHub")) {
        f.entry() {
            f.validateButton(title: _("Connect to API"), progress: _("Connecting..."), with: "serverAPIUrl,serverAPIProxy,credentialsId", method: "testGithubAccess")
        }
    }
    ...
    f.optionalBlock(title: _("Test Permissions to a Repository")) {
        f.entry() {
            f.validateButton(title: _("Check repo permissions"), progress: _("Checking..."), with: "serverAPIUrl,serverAPIProxy,credentialsId,repo", method: "checkRepoAccess")
        }
    }
    f.optionalBlock(title: _("Test adding comment to Pull Request")) {
        ...
        f.validateButton(title: _("Comment to issue"), progress: _("Commenting..."), with: "serverAPIUrl,serverAPIProxy,credentialsId,repo,issueId,message1", method: "testComment")
    }
    f.optionalBlock(title: _("Test updating commit status")) {
        ...
        f.validateButton(title: _("Update status"), progress: _("Updating..."), with: "serverAPIUrl,serverAPIProxy,credentialsId,repo,sha1,state,url,message2,context", method: "testUpdateStatus")
    }
}
...
f.advanced(title: _("Create API Token")) {
    f.entry(title: _("Create API Token")) {
        ...
        f.validateButton(title: _("Create Token"), progress: _("Creating..."), with: "serverAPIUrl,serverAPIProxy,credentialsId,username,password", method: "createApiToken")
    }
}
